...
- (void)configureForEvent:(Event *)event {
    ...
    {
        self.eventTypeLabel.text = kPFStringEventListTableViewCellLabelEventTypeAbsence;
        self.startDateLabel.text = @"";
        self.finishDateLabel.text = @"";
        self.intervalTimeLabel.text = @"";
    }
    else {
        
        if ([event isHoliday]) {
            self.eventTypeLabel.text = kPFStringEventListTableViewCellLabelEventTypeHoliday;
        }
        else {
            self.eventTypeLabel.text = kPFStringEventListTableViewCellLabelEventTypeSession;
        }
    ...
    self.timeBalanceLabel.text = [NSString stringWithTimeInterval:event.timeBalance];
}
